...
Single entrypoint that builds (DEFAULT, no args):
  ...
  - Static SVG font (Regular master) -> dist/fonts/ligatureluurs.svg
  - CSS bundle -> dist/fonts/ligatureluurs.css (copied from src/style/main.css)

Also supports:
  - --no-variable
      Build 3 normal static fonts instead of a variable font:
        dist/fonts/ligatureluurs-thin.{ttf,woff,woff2,svg}
        dist/fonts/ligatureluurs-regular.{ttf,woff,woff2,svg}
        dist/fonts/ligatureluurs-bold.{ttf,woff,woff2,svg}

  - --static
      Old behavior: build one single static font from --in
...
SVG font note:
- SVG fonts are a legacy format and do NOT support variable-font behavior.
- In variable-font mode, the exported .svg font is therefore a static export
  of the Regular master.

If you ever want the old single static behavior:
...
DEFAULT_GLYPH_SCRIPT = REPO_ROOT / "tools" / "generate-svgs.py"
...
def reorder_contours_to_reference(
    ...
    for i_ref in idxs:
        ...
        out.append(contours[best_i])  # type: ignore[index]
    return out
...
def build_ttf_from_svg_dir(
    ...
    # SVG export collectors:
    collect_outline_data_out: Optional[Dict[str, List[List[Tuple[int, int]]]]] = None,
    collect_hmtx_out: Optional[Dict[str, Tuple[int, int]]] = None,
    lsb_pad: int = 55,
    rsb_pad: int = 55,
):
    """
    ...
    if collect_outline_data_out is not None:
        collect_outline_data_out[".notdef"] = []
        collect_outline_data_out["space"] = []
    if collect_hmtx_out is not None:
        collect_hmtx_out[".notdef"] = hmtx[".notdef"]
        collect_hmtx_out["space"] = hmtx["space"]
    ...
    for gname in glyph_order:
        ...
        if found is None:
            ...
            if collect_outline_data_out is not None:
                collect_outline_data_out[gname] = []
            if collect_hmtx_out is not None:
                collect_hmtx_out[gname] = hmtx[gname]
            continue
        ...
        if collect_outline_data_out is not None:
            collect_outline_data_out[gname] = contours_fc
...
def style_slug(style: str) -> str:
    s = re.sub(r"[^a-z0-9]+", "-", style.lower()).strip("-")
    return s or "regular"


def contours_to_svg_path(contours: List[List[Tuple[int, int]]]) -> str:
    parts: List[str] = []
    for pts in contours:
        if len(pts) < 3:
            continue
...
def build_unicode_map_for_svg(
    cmap: Dict[int, str],
    ligatures: List[Tuple[List[str], str]],
) -> Dict[str, str]:
    by_glyph: Dict[str, str] = {}
    cp_by_glyph: Dict[str, int] = {}

    for cp, gname in cmap.items():
        if gname not in by_glyph:
            by_glyph[gname] = chr(cp)
        if gname not in cp_by_glyph:
            cp_by_glyph[gname] = cp

    for comps, lig_glyph in ligatures:
        seq_chars: List[str] = []
        ok = True
        for comp in comps:
            cp = cp_by_glyph.get(comp)
            if cp is None:
                ok = False
                break
            seq_chars.append(chr(cp))
        if ok and lig_glyph not in by_glyph:
            by_glyph[lig_glyph] = "".join(seq_chars)
...
def write_svg_font(
    ...
    hmtx_map: Dict[str, Tuple[int, int]],
) -> None:
    """
    Write a legacy static SVG font.
    In variable-font mode, call this with the Regular master.
    """
    out_path.parent.mkdir(parents=True, exist_ok=True)

    asc = int(font["hhea"].ascent) if "hhea" in font else int(M.BASE - M.CAP_TOP)
    desc = abs(int(font["hhea"].descent)) if "hhea" in font else int(M.DESC_END - M.BASE)
    linegap = int(font["hhea"].lineGap) if "hhea" in font else 0

    xheight = int(getattr(font["OS/2"], "sxHeight", int(round(M.XH)))) if "OS/2" in font else int(round(M.XH))
    capheight = int(getattr(font["OS/2"], "sCapHeight", int(round(M.BASE - M.CAP_TOP)))) if "OS/2" in font else int(round(M.BASE - M.CAP_TOP))
    weight_class = int(getattr(font["OS/2"], "usWeightClass", 400)) if "OS/2" in font else 400

    default_adv = mean_int(adv for adv, _lsb in hmtx_map.values()) if hmtx_map else int(M.UPM * 0.6)

    full_name = f"{family} {style}".strip()
    font_id = re.sub(r"[^A-Za-z0-9_-]+", "-", f"{filename_base}-{style_slug(style)}").strip("-") or filename_base

    unicode_map = build_unicode_map_for_svg(cmap, ligatures)

    def esc_attr(s: str) -> str:
        return html.escape(s, quote=True)

    def glyph_tag(gname: str, unicode_value: Optional[str]) -> str:
        adv, _lsb = hmtx_map.get(gname, (default_adv, 0))
        d = contours_to_svg_path(outlines.get(gname, []))
        attrs = [
            f'glyph-name="{esc_attr(gname)}"',
            f'horiz-adv-x="{int(adv)}"',
        ]
        if unicode_value is not None:
            attrs.append(f'unicode="{esc_attr(unicode_value)}"')
        if d:
            attrs.append(f'd="{esc_attr(d)}"')
        return "      <glyph " + " ".join(attrs) + " />"

    missing_d = contours_to_svg_path(outlines.get(".notdef", []))
    missing_adv = int(hmtx_map.get(".notdef", (default_adv, 0))[0])

    lines: List[str] = [
        '<?xml version="1.0" encoding="UTF-8"?>',
        '<svg xmlns="http://www.w3.org/2000/svg">',
        "  <defs>",
        f'    <font id="{esc_attr(font_id)}" horiz-adv-x="{int(default_adv)}">',
        (
            f'      <font-face '
            f'font-family="{esc_attr(family)}" '
            f'font-style="normal" '
            f'font-weight="{weight_class}" '
            f'units-per-em="{M.UPM}" '
            f'ascent="{asc}" '
            f'descent="{desc}" '
            f'cap-height="{capheight}" '
            f'x-height="{xheight}" '
            f'alphabetic="0" '
            f'underline-position="-100" '
            f'underline-thickness="50" '
            f'panose-1="2 0 0 0 0 0 0 0 0 0" />'
        ),
        f'      <metadata>{esc_attr(full_name)} — Version {version} — static SVG font export</metadata>',
    ]

    if missing_d:
        lines.append(f'      <missing-glyph horiz-adv-x="{missing_adv}" d="{esc_attr(missing_d)}" />')
    else:
        lines.append(f'      <missing-glyph horiz-adv-x="{missing_adv}" />')

    # Keep .notdef as a named glyph too
    if ".notdef" in glyph_order:
        lines.append(glyph_tag(".notdef", None))

    for gname in glyph_order:
        if gname == ".notdef":
            continue
        unicode_value = unicode_map.get(gname)
        lines.append(glyph_tag(gname, unicode_value))
...
def main() -> None:
    ...
    ap.add_argument(
        "--formats",
        nargs="+",
        default=["ttf", "woff", "woff2", "svg"],
        choices=["ttf", "woff", "woff2", "svg"],
    )
    ...
    ap.add_argument(
        "--no-variable",
        action="store_true",
        help="Build separate normal static fonts (Thin/Regular/Bold) instead of one variable font.",
    )

    ap.add_argument("--glyph-script", type=Path, default=DEFAULT_GLYPH_SCRIPT,
                    help="Path to tools/generate-svgs.py (assumed by default).")
    ...
    ap.add_argument("--no-generate", action="store_true",
                    help="Do NOT call tools/generate-svgs.py; assume master SVG dirs already exist in build-dir.")
    args = ap.parse_args()
    ...
    binary_formats = [fmt for fmt in formats if fmt != "svg"]

    # -------------------------
    # Static mode (old behavior)
    # -------------------------
    if args.static:
        outline_data: Dict[str, List[List[Tuple[int, int]]]] = {}
        hmtx_data: Dict[str, Tuple[int, int]] = {}

        font, go, cmap, liga, _cc = build_ttf_from_svg_dir(
            svg_dir=args.in_dir,
            family=args.family,
            style=args.style,
            ...
            collect_outline_data_out=outline_data if want_svg else None,
            collect_hmtx_out=hmtx_data if want_svg else None,
            lsb_pad=int(args.lsb_pad),
            rsb_pad=int(args.rsb_pad),
        )
        save_font_with_formats(font, base_out, binary_formats)

        if want_svg:
            write_svg_font(
                out_path=base_out.with_suffix(".svg"),
                font=font,
                family=args.family,
                style=args.style,
                version=args.version,
    ...
    # -------------------------
    # Variable mode / static-masters mode
    # -------------------------
    ...
    master_fonts: List[object] = [None, None, None]  # type: ignore[assignment]
    master_outline_data: List[Dict[str, List[List[Tuple[int, int]]]]] = [{}, {}, {}]
    master_hmtx_data: List[Dict[str, Tuple[int, int]]] = [{}, {}, {}]
    ...
    reg_font, glyph_order, cmap, ligatures, reg_counts = build_ttf_from_svg_dir(
        ...
        collect_outline_data_out=master_outline_data[regular_idx] if want_svg else None,
        collect_hmtx_out=master_hmtx_data[regular_idx] if want_svg else None,
        lsb_pad=int(args.lsb_pad),
        rsb_pad=int(args.rsb_pad),
    )
    reg_font.save(master_ttf_paths[regular_idx])
    print(f"Wrote master: {master_ttf_paths[regular_idx]}")
    master_fonts[regular_idx] = reg_font

    all_counts: List[Dict[str, int]] = [None, reg_counts, None]  # type: ignore[list-item]
    ...
    for idx in (0, 2):
        f, _go2, _cmap2, _liga2, cc = build_ttf_from_svg_dir(
            ...
            collect_outline_data_out=master_outline_data[idx] if want_svg else None,
            collect_hmtx_out=master_hmtx_data[idx] if want_svg else None,
            lsb_pad=int(args.lsb_pad),
            rsb_pad=int(args.rsb_pad),
        )
        ...
        master_fonts[idx] = f
        all_counts[idx] = cc  # type: ignore[index]
    ...
    if args.no_variable:
        for idx, style in enumerate(styles):
            out_base = out_dir / f"{args.filename}-{style_slug(style)}"
            static_font = master_fonts[idx]
            save_font_with_formats(static_font, out_base, binary_formats)
            ...
            if want_svg:
                write_svg_font(
                    ...
                    style=style,
                    version=args.version,
                    filename_base=f"{args.filename}-{style_slug(style)}",
                    glyph_order=glyph_order,
                    cmap=cmap,
                    ligatures=ligatures,
                    outlines=master_outline_data[idx],
                    hmtx_map=master_hmtx_data[idx],
                )
    ...
    save_font_with_formats(vf, base_out, binary_formats)

    # 6b) Save static SVG font from Regular master
    if want_svg:
        write_svg_font(
            out_path=base_out.with_suffix(".svg"),
            font=reg_font,
            ...
            cmap=cmap,
            ligatures=ligatures,
            outlines=master_outline_data[regular_idx],
            hmtx_map=master_hmtx_data[regular_idx],
        )
