...
Key improvements:
- PROPORTIONAL SPACING: advance widths + sidebearings are computed from glyph outline bbox,
  NOT from the SVG viewBox width (which was effectively monospace per class).
- For variable fonts, a single spacing plan is computed using the MAX outline width across masters,
  so metrics stay stable across the wght axis (no jitter, no HVAR dependency).
"""
...
def normalize_contours_to_fontcoords(
    ...
    for pts in contours_svg:
        if len(pts) < 3:
            continue
        tpts = [(x, baseline_svg - y) for x, y in pts]  # SVG y-down -> font y-up
        if resample_n > 0:
...
def compute_spacing_plan(
    ...
    Shifts outlines per-master so xMin == lsb.
    """

    def sb_for_width(w: int) -> int:
        # A simple, robust formula that works well for monoline rounded ends:
        # start with a base, add a small amount for wider glyphs, clamp.
        sb = int(round(sb_base + sb_scale * float(w)))
        sb = max(sb, sb_min)
        sb = min(sb, sb_max)
        return sb

    # Track max content width across masters for each glyph
    ...
    for g in glyph_order:
        ...
        # Keep advances sane for empty glyphs / edge cases
        adv = max(adv, sb * 2 + 120)
        plan[g] = (adv, sb)
...
def build_ttf_from_svg_dir(
    ...
    # Determine glyph_order/cmap/ligatures either from forced inputs or from this directory
    ...
    for gname in glyph_order:
        ...
        if not svg_path:
            # Missing: empty glyph with metrics from plan or a fallback
        ...
        _vb_w, _vb_h, d = parse_svg_viewbox_and_path(svg_path)
        ...
        if spacing_plan and gname in spacing_plan:
            ...
            # Fallback: keep old advance behavior, but FIX LSB consistency
            # (LSB should match xMin in glyf).
            adv = int(round(_vb_w))
            lsb = int(xMin)

        # Shift outline so xMin == lsb (keeps metrics consistent and predictable)
        dx = int(lsb) - int(xMin)
...
def main() -> None:
    ...
    # Spacing tuning (optional; defaults should already look much nicer)
    ...
    if args.static:
        ...
        plan = compute_spacing_plan(
            ...
            resample=0,  # static doesn't need resample for interpolation; but bbox still ok
    ...
    # 2) Inventory from Regular master for stable glyph order/maps
    regular_idx = 1
    glyph_order, cmap, ligatures = collect_inventory(master_svg_dirs[regular_idx])

    # 3) Compute a single spacing plan based on MAX outline widths across masters
    plan = compute_spacing_plan(
    ...
    # 4) Build masters using the same glyph order/maps/spacing plan
    ...
    axis_name = axis.name  # designspace locations use axis.name, not axis.tag
