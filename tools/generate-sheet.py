...
def parse_glyph_svg(path: Path) -> Optional[Tuple[float, float, str]]:
    ...
    # Generated SVG uses xmlns="http://www.w3.org/2000/svg" => namespaced tags in ElementTree.
    # We'll handle both namespaced and non-namespaced just in case.
...
def svg_glyph_instance(d: str, x_sheet: float, baseline_sheet: float, scale: float, m: Metrics) -> str:
    ...
    """
    # If d is empty, render nothing (still keep layout).
    if not d.strip():
        return ""
...
def render_cell(
    *,
    ...
    if show_guides:
        x0 = x_cell + 10
        x1 = x_cell + cell_w - 10
        # y positions for key lines, relative to glyph y=0
...
def render_wrapped_sample(
    *,
    ...
    # Normalize whitespace for wrapping
...
def main() -> None:
    ...
    # Load glyph paths for A–Z and a–z
    ...
    max_glyph_w = max([w for w, _ in upper.values()] + [w for w, _ in lower.values()] + [m.CAP_W, m.LC_W])
    ...
    # Compute sheet width (based on the grids)
    ...
    # A mixed glyph map for sample lines
    ...
    frag, y = render_wrapped_sample(
        title="",
        text="HAMBURGEFONTSIV  hamburgefontsiv",
        glyphs=mixed,
        x=samples_x,
        y=y,
        max_w=samples_max_w,
